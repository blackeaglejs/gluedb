...
class RenewalPoliciesFileAssisted
  ...
  def process
    ...
    policies_by_subscriber.each do |subscriber, policies|
      ...
      # policies.each do |p|
      #   next if !p.plan.hios_plan_id.match(/-01/)
      ...
      zohebs_renewal_ids = File.new("renewal_ids_assisted_#{timestamp}.txt", "w")
  ...
  def policies_by_subscriber
    ...
    plans = hplans1 + hplans2 + hplans3 + hplans4 + hplans5 + hplans6
